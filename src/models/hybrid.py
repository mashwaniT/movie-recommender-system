...
def hybrid_recommendations(user_id, movie_title, movies_df, svd_model, nearest_neighbors_model, tfidf_matrix, k=10, alpha=0.5):
    ...
    _, indices = nearest_neighbors_model.kneighbors(tfidf_matrix[movie_index].reshape(1, -1), n_neighbors=k+1)

    content_based_similar_movies = movies_df.iloc[indices.flatten()]
    content_based_similar_movies['cosine_similarity'] = nearest_neighbors_model.kneighbors(tfidf_matrix[movie_index].reshape(1, -1), n_neighbors=k+1)[1][:, 1:]
    content_based_similar_movies = content_based_similar_movies[content_based_similar_movies.index != movie_index]
    ...
    content_based_similar_movies["cf_predictions"] = cf_predictions

    # Step 3: Combine both sets of scores
    content_based_similar_movies["hybrid_score"] = (
